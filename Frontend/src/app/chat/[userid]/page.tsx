    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="w-1/4 bg-white border-r border-gray-300">
       ...
       <div  className="overflow-y-auto bg-slate-950 h-screen p-3 mb-9 pb-20">
          ...
          <div onClick={()=>{setSelectedSidebar(user._id), setFriend(user._id),setUsername(user.username),setuserAvartar(user.avatarUrl)}}  key={user._id} typeof="checkbox" className={` flex items-center  mb-4 cursor-pointer text-white hover:text-black hover:bg-gray-400 active:bg-gray-50  p-2 rounded-md  ${selectedSidebar === user._id ? 'bg-gray-400' : ''}`}>
            <div className="w-12 h-12 rounded-full mr-3 ">
              <img
                src={user.avatarUrl ? user.avatarUrl : `https://placehold.co/200x/FFFAF0/0A0A0A.svg?text=${user.username.charAt(0).toUpperCase()}&font=Lato`} alt="User Avatar" className="w-12 h-12 rounded-full"/>
            </div>
